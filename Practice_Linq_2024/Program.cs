...
namespace Practice_Linq_2024
{
    internal class Program
    {
        ...
        static void Query5(List<FootballGame> games)
        {
            ...
            var selectedGames = games.Where(p => p.Tournament == "UEFA Euro qualification" && (p.City == "Kyiv" || p.City == "Kharkiv") && p.Home_score > p.Away_score).OrderBy(p => p.Date);  // Корегуємо запит !!!
        ...
        static void Query7(List<FootballGame> games)
        {
            ...
            FootballGame g = games.First(p => p.Date.Year == 2023 && (p.Home_team == "Ukraine" && p.Home_score > p.Away_score || p.Away_team == "Ukraine" && p.Away_score > p.Home_score));   // Корегуємо запит !!!
        ...
        static void Query8(List<FootballGame> games)
        {
            ...
            var selectedGames = games.Where(p => p.Date.Year == 2012 && p.Tournament == "UEFA Euro" && p.Country == "Ukraine").Select(p => new
            {
                MatchYear = p.Date.Year,
                Team1 = p.Home_team,
                Team2 = p.Away_team,
                Goals = p.Home_score + p.Away_score
            });   // Корегуємо запит !!!
        ...
        static void Query9(List<FootballGame> games)
        {
            ...
            var selectedGames = games.Where(p => p.Date.Year == 2023 && p.Tournament == "UEFA Nations League").Select(p => new
            {
                MatchYear = p.Date.Year,
                Game = $"{p.Home_team} - {p.Away_team}",
                Result = p.Home_score > p.Away_score ? "Win" :
                         p.Home_score < p.Away_score ? "Loss" : "Draw"
            });   // Корегуємо запит !!!
        ...
        static void Query10(List<FootballGame> games)
        {
            ...
            var selectedGames = games.Where(p => p.Date.Year == 2023 && p.Date.Month == 7 && p.Tournament == "Gold Cup").Skip(4).Take(6);    // Корегуємо запит !!!
